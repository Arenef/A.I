...
class VisualAC3(ac3):
    ...
    def rm_inconsistent_values(self, Xi, Xj):
        ...
        for x in to_remove:
            Xi.domains.remove(x)
            if 'on_fc_prune' in self.callbacks:
                self.callbacks['on_fc_prune'](Xi.name, x)
        ...
        for color in list(current_var.domains):
            ...
            if self.is_valid(current_var, color):
                ...
                for var in self.variables:
                    old_domain = domain_backup[var.name]
                    for color_restored in old_domain:
                        if color_restored not in var.domains:
                            if 'on_fc_restore' in self.callbacks:
                                self.callbacks['on_fc_restore'](var.name, color_restored)
                    var.domains = list(old_domain)
...
class VisualMinConflicts(min_conflicts):
    def __init__(self, variables, max_steps=1000, callbacks=None):
        super().__init__(variables, max_steps)
        self.callbacks = callbacks or {}

    def solve(self):
        return self.min_conflicts(self.max_steps)
    ...
    def min_conflicts(self, max_steps):
        ...
        if 'on_mc_init' in self.callbacks:
            self.callbacks['on_mc_init'](current)
        ...
        for i in range(1, max_steps + 1):
            ...
            if 'on_mc_step' in self.callbacks:
                self.callbacks['on_mc_step'](i, var.name, old_val, value, len(conflicted_variables))
...
class GraphColoringApp:
    def __init__(self, root):
        ...
        for algo in algos:
            ...
            rb.pack(anchor="w", padx=25, pady=4, fill="x")
        
        self.lbl_group = tk.Label(left_panel, text="Nhóm: CSP / Quay lui", font=("Segoe UI", 9, "italic"), fg="#a29bfe", bg="#242541", wraplength=200, justify="left")
        self.lbl_group.pack(anchor="w", padx=20, pady=(15, 25))
        ...
        self.canvas = tk.Canvas(canvas_card, width=650, height=490, bg="#11111e", highlightthickness=0)
        ...
        self.solution_area = tk.Text(center_panel, height=5, bg="#11111e", fg="#e5e7eb", font=("Segoe UI", 10), bd=0, highlightthickness=1, highlightbackground="#374151")
        ...
        self.log_area.tag_config("ac3", foreground="#c084fc")
    ...
    def run_solvers(self):
        ...
        try:
            callbacks = {
                ...
                'on_mc_init': self.cb_on_mc_init,
                'on_mc_step': self.cb_on_mc_step
            }
            ...
            if algo == "Backtracking":
                ...
                problem = VisualAC3(variables=variables_list, callbacks=callbacks)
                solution = problem.solve(0)
            elif algo == "Min-Conflicts":
                problem = VisualMinConflicts(variables=variables_list, callbacks=callbacks)
            ...
            if success and solution:
                ...
                res_str = "\n".join([f"• {var_name}: {color}" for var_name, color in solution.items()])
    ...
    def cb_on_fc_prune(self, var_name, color_name):
        algo = self.algo_var.get()
        tag = "ac3" if algo == "AC-3" else "fc"
        prefix = "AC-3" if algo == "AC-3" else "FC"
        self.log(f"   [{prefix}] Loại bỏ màu {color_name} khỏi miền của {var_name}", tag)

    def cb_on_fc_restore(self, var_name, color_name):
        algo = self.algo_var.get()
        tag = "ac3" if algo == "AC-3" else "fc"
        prefix = "AC-3" if algo == "AC-3" else "FC"
        self.log(f"   [{prefix}] Khôi phục màu {color_name} cho miền của {var_name}", tag)

    def cb_on_mc_init(self, initial_assignment):
        self.check_control_state()
        self.log("=== KHỞI TẠO CẤU HÌNH NGẪU NHIÊN ===", "header")
        for var_name, color_name in initial_assignment.items():
            tk_color = color_map[color_name]
            current_assignment[var_name] = tk_color
            self.log(f"-> {var_name}: Gán màu {color_name}", "info")
        self.draw_map()
        self.update_progress()
        time.sleep(self.delay)

    def cb_on_mc_step(self, step, var_name, old_color_name, new_color_name, num_conflicts):
        self.check_control_state()
        tk_color = color_map[new_color_name]
        current_assignment[var_name] = tk_color
        self.draw_map()
        self.log(f"\n[Bước {step}]: Chọn quận xung đột {var_name}", "header")
        self.log(f"   Thay đổi màu: {old_color_name} -> {new_color_name}", "info")
        tag = "violate" if num_conflicts > 0 else "valid"
        self.log(f"   Số quận còn xung đột: {num_conflicts}", tag)
        self.update_progress()
        time.sleep(self.delay)
    ...
    def on_algo_change(self, event=None):
        algo = self.algo_var.get()
        if algo == "Backtracking":
            self.lbl_group.config(text="Nhóm: CSP / Quay lui")
        elif algo == "Forward Checking":
            self.lbl_group.config(text="Nhóm: CSP / Lan truyền ràng buộc (FC)")
        elif algo == "AC-3":
            self.lbl_group.config(text="Nhóm: CSP / Duy trì tính nhất quán cung (MAC)")
        elif algo == "Min-Conflicts":
            self.lbl_group.config(text="Nhóm: CSP / Tối ưu cục bộ (Local Search)")
